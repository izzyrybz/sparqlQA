...
def get_subgraph(subject, endpoint_url):
    query = """
    CONSTRUCT {
        <%s> ?p ?o .
        ?s ?p <%s> .
...
def qg(linker, kb, parser, qapair, force_gold=True):
    ...
    r = requests.get("http://localhost:3030/dbpedia/query",  params={"query": qapair.sparql.query.replace("https", "http")})
    ...
    if entities is None or ontologies is None:
        print("no entities or ontologies found")
        return "-Linker_failed", []


    print("start finding the minimal subgraph")
    print(entities,ontologies)
    endpoint_url = "http://localhost:3030/dbpedia/query"
    subject = "<http://example.org/subject>"

    subgraph = get_subgraph(subject, endpoint_url)
    print(subgraph.decode())
...
if __name__ == "__main__":
    ...
    #This part creates the uris and names the raw uris 
    ...
    #this is for the DBpedia
    kb = parser.kb

    #from common.Stats
    stats = Stats()

    #froim linker. GoldLinker I think this could be phrasemapping
    linker = GoldLinker()
